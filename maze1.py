...
## 브금이랑 효과음
sound_file = "Undertale-Megalovania-Piano-Ver-.wav"
bomb_file = "bomb.wav"
urgent_file = "HORROR.wav"
mixer = pygame.mixer
mixer.init()
...
class App:
    ...
    def on_init(self):
        ...
        self._running = True
        self._block_surf = pygame.image.load("block%d.jpg" %cnt).convert()
        self._ground_surf = pygame.image.load("ground%d.jpg" %cnt).convert()
        if char == 1:
           self._image_surf = pygame.image.load("squ%d.jpg" %cnt).convert()
        elif char == 2:
           self._image_surf = pygame.image.load("fire%d.jpg" %cnt).convert()
        elif char == 3:
           self._image_surf = pygame.image.load("pika%d.jpg" %cnt).convert()
        self._question_surf = pygame.image.load("question%d.jpg" %cnt).convert()
